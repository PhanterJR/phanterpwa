...
class Datepickers():
    ...
    def __init__(self, element, parameters):
        ...
        if jQuery(element).length > 0:
            if jQuery(element)[0].hasAttribute("phanterpwa-datetimepicker-iso"):
                self.selected_date = __new__(Date(jQuery(element).attr("phanterpwa-datetimepicker-iso")))
    ...
    def close(self):
        jQuery(self.element).find(".phanterpwa-fixed-fulldisplay").removeClass("enabled")
        
        setTimeout(
            lambda: jQuery(self.element).find(".phanterpwa-fixed-fulldisplay").remove(),
            1000
        )
    ...
    def start(self):
        ...
        summary = DIV(
            ...
            DIV(
                DIV(
                    SPAN(self._zfill(sday, 2)), ", ", SPAN(self.months[smonth]),
        ...
        def open_panel(el):
            setTimeout(
                lambda: jQuery(self.element).find(".phanterpwa-fixed-fulldisplay").addClass("enabled"),
                100
            )

        if jQuery(self.element).has(".phanterpwa-fixed-fulldisplay").length == 1:
            jQuery(self.element).find(".phanterpwa-fixed-fulldisplay").remove()
            jQuery(self.element).append(centralizer.xml())
            jQuery(self.element).find(".phanterpwa-fixed-fulldisplay").addClass("enabled")
        else:
            jQuery(self.element).append(centralizer.xml())
            open_panel()

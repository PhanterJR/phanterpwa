// Transcrypt'ed from Python, 2020-04-10 03:44:32
...
export var Index =  __class__ ('Index', [gatehandler.Handler], {
	...
	get _xml_projects_list () {return __get__ (this, function (self, json) {
		...
		var table = XTABLE ('projects-table', XTRH ('projects-table-head', ...['Project Name', 'Diretory', 'Status'], DIV (I (__kwargtrans__ ({_class: 'fas fa-plus'})), __kwargtrans__ (dict ({'_phanterpwa-way': 'project/novo', '_class': 'icon_button wave_on_click'})))));
		for (var x of json.projects_list) {
			var btn_lbl = 'run';
			if (x [2] == 'running') {
				var btn_lbl = 'stop';
			}
			table.append (XTRD ('projects-table-data-{0}'.format (x [0]), x [0], x [1], SPAN (x [2].capitalize (), __kwargtrans__ (dict ({'_class': 'project_status_content {0}'.format (x [2]), '_id': 'project_status_{0}'.format (x [0]), '_data-path': x [1]}))), widgets.MenuBox ('drop_{0}'.format (x [0]), __kwargtrans__ ({xml_menu: UL (LI (btn_lbl.capitalize (), __kwargtrans__ (dict ({'_id': 'btn_run_stop_project_{0}'.format (x [0]), '_class': 'btn_run_stop_project', '_data-path': x [1]}))), LI ('View', __kwargtrans__ (dict ({'_class': 'botao_editar_role', '_phanterpwa-way': 'project/{0}/view'.format (x [0])}))), LI ('Delete', __kwargtrans__ (dict ({'_class': 'botao_editar_role', '_phanterpwa-way': 'project/{0}/delete'.format (x [0])}))), __kwargtrans__ (dict ({'data-menubox': 'drop_{0}'.format (x [0]), '_class': 'dropdown-content'}))), onOpen: self._bind_btn_run_stop}))));
		}

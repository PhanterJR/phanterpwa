...
class Index(gatehandler.Handler):
    def __init__(self, request):
        self.list_icons = list_icons
        self._cont_icon_list = 0
        self._max_icon_list = len(self.list_icons)
    ...
    def initialize(self):
        ...
        setTimeout(lambda: self._add_content(0), 300)

    def _sampled(self, value):
        html = DIV(
            DIV(
                DIV(
                    widgets.MenuBox(
    ...
    def _add_content(self, v):
        ...
        start = v
        stop = v + 20
        if stop > self._max_icon_list:
            stop = self._max_icon_list
        cont = 0
        for x in range(start, stop):
            h = self._sampled(self.list_icons[x])
            html.append(h)
            cont += 1
        else:
            if v == 0:
                html.html_to("#icon_awesome_container")
                self._async_add_content(cont)
            else:
                html.append_to("#icon_awesome_container")
                self._async_add_content(stop)

    def _async_add_content(self, cont):
        setTimeout(lambda: self._add_content(cont))

// Transcrypt'ed from Python, 2020-03-29 19:45:21
...
import * as gallery from './phanterpwa.apptools.components.gallery.js';
import * as left_bar from './phanterpwa.apptools.components.left_bar.js';
import * as top_slide from './phanterpwa.apptools.components.top_slide.js';
import * as modal from './phanterpwa.apptools.components.modal.js';
var __name__ = 'phanterpwa.apptools.components.auth_user';
...
export var ModalLogin =  __class__ ('ModalLogin', [modal.Modal], {
	...
	get submit () {return __get__ (this, function (self) {
		...
		self.clear_errors ();
		window.PhanterPWA.login ($ ('#phanterpwa-widget-input-login-csrf_token').val (), $ ('#phanterpwa-widget-input-input-login-email').val (), $ ('#phanterpwa-widget-input-input-login-password').val (), $ ('#phanterpwa-widget-checkbox-input-login-remember_me').prop ('checked'), __kwargtrans__ ({callback: self.after_submit}));
	});}
});
...
export var ModalRequestPassword =  __class__ ('ModalRequestPassword', [modal.Modal], {
	...
	get __init__ () {return __get__ (this, function (self, target_element) {
		...
		var widget_email = forms.FormWidget ('request_password', 'email', __kwargtrans__ (dict ({'type': 'string', 'label': I18N ('E-Mail'), 'phanterpwa': dict ({'validators': ['IS_EMAIL']}), '_class': 'p-col w1p100'})));
		var last_auth_user = window.PhanterPWA.get_last_auth_user ();
		if (last_auth_user !== null) {
			var widget_email = forms.FormWidget ('request_password', 'email', __kwargtrans__ (dict ({'type': 'string', 'label': I18N ('E-Mail'), 'phanterpwa': dict ({'validators': ['IS_EMAIL']}), 'value': last_auth_user.email, '_class': 'p-col w1p100'})));
		}
...
export var AlertActivationAccount =  __class__ ('AlertActivationAccount', [top_slide.TopSlide], {
	...
	get submit () {return __get__ (this, function (self) {
		...
		self.element_target = $ (self.target_selector);
		self.clear_errors ();
		window.PhanterPWA.activation_account (self.element_target.find ('#phanterpwa-widget-input-activation-csrf_token').val (), self.element_target.find ('#phanterpwa-widget-input-input-activation-activation_code').val (), __kwargtrans__ ({callback: self.after_submit}));
	});},
...
export var LeftBarAuthUserLogin =  __class__ ('LeftBarAuthUserLogin', [left_bar.LeftBarUserMenu], {
	...
	get __init__ () {return __get__ (this, function (self) {
		...
		self.addSubmenu ('lock', I18N ('Lock', __kwargtrans__ (dict ({'_pt-br': 'Bloquear'}))), __kwargtrans__ ({_class: 'command_user'}));
...
export var Profile =  __class__ ('Profile', [handler.GateHandler], {
	...
	get reload () {return __get__ (this, function (self) {
		...
		self.GalleryInput = gallery.GalleryInput ('#profile-image-user-container', __kwargtrans__ (dict ({'cutter': true, 'current_image': user_image})));
...
export var Lock =  __class__ ('Lock', [handler.GateHandler], {
	__module__: __name__,
	...
	get submit () {return __get__ (this, function (self) {
		...
		formdata.append ('csrf_token', $ ('#form-lock #phanterpwa-widget-input-lock-csrf_token').val ());
		var login_password = '{0}:{1}'.format (window.btoa ($ ('#form-lock #phanterpwa-widget-input-lock-email').val ()), window.btoa ($ ('#form-lock #phanterpwa-widget-input-lock-password').val ()));
		formdata.append ('authorization', login_password);
		var remember_me = false;
		if ($ ('#form-lock #phanterpwa-widget-input-lock-remember_me').prop ('checked')) {
			var remember_me = true;
		}
		formdata.append ($ ('#form-lock #phanterpwa-widget-input-lock-remember_me').attr ('name'), remember_me);
	...
	get after_confirm_lock () {return __get__ (this, function (self, data, ajax_status) {
		...
		if (ajax_status == 'success') {
			...
			$ ('#form-lock #phanterpwa-widget-input-lock-email').val (self.last_auth_user.email);
			if (self.last_auth_user.remember_me) {
				$ ('#form-lock #phanterpwa-widget-input-lock-remember_me').attr ('checked', 'checked');
			}
...
export var TwoFactor =  __class__ ('TwoFactor', [handler.GateHandler], {
	__module__: __name__,
	get initialize () {return __get__ (this, function (self) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
...
//# sourceMappingURL=phanterpwa.apptools.components.auth_user.map
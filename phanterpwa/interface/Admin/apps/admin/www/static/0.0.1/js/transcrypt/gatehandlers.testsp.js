// Transcrypt'ed from Python, 2020-04-24 10:42:26
...
var __name__ = 'gatehandlers.testsp';
...
export var Index =  __class__ ('Index', [gatehandler.Handler], {
	...
	get initialize () {return __get__ (this, function (self) {
		...
		var html = CONCATENATE (DIV (DIV (DIV (DIV ('TEST PHANTERPWA', __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), __kwargtrans__ ({_class: 'phanterpwa-breadcrumb-wrapper'})), __kwargtrans__ ({_class: 'p-container'})), __kwargtrans__ ({_class: 'title_page_container card'})), DIV (DIV (DIV (DIV (widgets.Preloaders ('preload_android'), __kwargtrans__ ({_style: 'width:100%; text-align: center; padding-top: 100px; padding-bottom: 100px;'})), __kwargtrans__ ({_id: 'applications_container'})), __kwargtrans__ ({_class: 'card p-row e-padding_10'})), __kwargtrans__ ({_class: 'phanterpwa-container p-container'})));
		...
		var BackButton = left_bar.LeftBarButton ('back_developer', 'Voltar', I (__kwargtrans__ ({_class: 'fas fa-arrow-circle-left'})), __kwargtrans__ (dict ({'_phanterpwa-way': 'developer', 'position': 'top', 'ways': [(function __lambda__ () {
			...
			return window.PhanterPWA.get_current_way ().startswith ('test_phanterpwa');
		})]})));
		window.PhanterPWA.Components ['left_bar'].add_button (BackButton);
		window.PhanterPWA.GET (__kwargtrans__ (dict ({'url_args': ['api', 'automation', 'test_list'], 'onComplete': self._after_get_test_list})));
	});},
	...
	get _xml_test_list () {return __get__ (this, function (self, json) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
			if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
				var __allkwargs0__ = arguments [__ilastarg0__--];
		...
		var table = XTABLE ('tests-table', XTRH ('tests-table-head', ...['Test', 'status']));
		for (var x of json.test_list) {
			table.append (XTRD ('tests-table-data-{0}'.format (x), x.py_split ('_') [1].capitalize (), DIV (SPAN ('Testing...', __kwargtrans__ ({_style: 'color: orange;'})), __kwargtrans__ (dict ({'_data-test': x, '_class': 'test_phanterpwa_status'})))));
		}
		...
		window.PhanterPWA.WS.send (dict ({'authorization': self.admin_authorization, 'command': 'test_phanterpwa'}));
	});},
	get _onwsmessage () {return __get__ (this, function (self, ev) {
		...
		if (json.command == 'test_phanterpwa') {
			if (json.status == 206) {
				$ (".test_phanterpwa_status[data-test='{0}']".format (json.test)).html ((json.result ? "<span style='color: red'>FAIL</span>" : "<span style='color: green'>PASS</span>"));
			}
	...
	get _after_get_test_list () {return __get__ (this, function (self, data, ajax_status) {
		if (arguments.length) {
			var __ilastarg0__ = arguments.length - 1;
			if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
				var __allkwargs0__ = arguments [__ilastarg0__--];
...
//# sourceMappingURL=gatehandlers.testsp.map
...
class TestProjectConfig(unittest.TestCase):
    def test0_documentation_example(self):
        ...
        s_cfg = interpolate(sample_cfg, {
            ...
            "app01": os.path.join(path_dir, "apps", "app01", "www").replace("\\", "\\\\"),
            "app02": os.path.join(path_dir, "apps", "app02", "www").replace("\\", "\\\\")
        })
        self.assertEqual(
            string_cfg,
            s_cfg
        ...
        # inis
        self.assertTrue(isinstance(cfg.api_ini, configparser.ConfigParser))
        self.assertTrue(isinstance(cfg.project_ini, configparser.ConfigParser))
        self.assertTrue(isinstance(cfg.secret_ini, configparser.ConfigParser))
        self.assertTrue(isinstance(cfg.apps_ini["app01"], configparser.ConfigParser))
        self.assertTrue(isinstance(cfg.apps_ini["app02"], configparser.ConfigParser))
        # save
        self.assertEqual(
            cfg['PROJECT']['title'],
            "PhanterPWA"
